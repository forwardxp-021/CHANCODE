...
_dll_search_handles = []
_loaded_dll_path: Path | None = None
global_dll_path: Path | None = None
...
def _candidate_dll_paths() -> list[Path]:
    ...
    env_path = (os.getenv("TDX_DLL_PATH") or os.getenv("TPYTHCLIENT_DLL") or "").strip()
    if env_path:
        candidates.append(Path(env_path))

    env_dir = (os.getenv("TDX_DLL_DIR") or os.getenv("TDX_CLIENT_DIR") or "").strip()
    if env_dir:
        env_base = Path(env_dir)
        candidates.extend(
            [
                env_base / "TPythClient.dll",
                env_base / "PYPlugins" / "TPythClient.dll",
                env_base / "PYPlugins" / "user" / "TPythClient.dll",
                env_base / "chancode" / "TPythClient.dll",
            ]
        )
    ...
    for root in (Path(r"C:\\new_tdx64"), Path(r"C:\\new_tdx")):
        if root.exists():
            candidates.extend(
                [
                    root / "TPythClient.dll",
                    root / "PYPlugins" / "TPythClient.dll",
                    root / "PYPlugins" / "user" / "TPythClient.dll",
                ]
            )

    def _collect_registry_install_roots() -> list[Path]:
        if winreg is None:
            return []
        roots: list[Path] = []
        registry_hives = (
        ...
        for hive, subpath in registry_hives:
            try:
                        ...
                        try:
                            ...
                            with winreg.OpenKey(key, subkey_name) as subkey:
                                try:
                                    ...
                                    display_name = ""
                                try:
                                    install_location = str(winreg.QueryValueEx(subkey, "InstallLocation")[0]).strip()
                                except Exception:
                                    install_location = ""
                                if (
                                    install_location
                                    and ("通达信" in display_name or "Tdx" in display_name or "TDX" in display_name)
                                ):
    ...
    for root_path in _collect_registry_install_roots():
        candidates.extend(
            [
                root_path / "TPythClient.dll",
                root_path / "PYPlugins" / "TPythClient.dll",
                root_path / "PYPlugins" / "user" / "TPythClient.dll",
                root_path / "chancode" / "TPythClient.dll",
            ]
        )

    candidates.extend(
        [
            base_dir / "TPythClient.dll",
            base_dir / "PYPlugins" / "TPythClient.dll",
            base_dir / "chancode" / "TPythClient.dll",
            base_dir / "PYPlugins" / "user" / "TPythClient.dll",
            Path(__file__).resolve().parent / "TPythClient.dll",
            Path.cwd() / "TPythClient.dll",
        ]
    )
    ...
    for path in _candidate_dll_paths():
        ...
        try:
            if hasattr(os, "add_dll_directory"):
                _dll_search_handles.append(os.add_dll_directory(str(path.parent)))
        except Exception:
            pass

        try:
            dll_obj = ctypes.CDLL(str(path))
            _loaded_dll_path = path
            print(f"已加载 TPythClient.dll: {path}")
            return dll_obj
        except OSError as exc:
            errors.append(f"{path}: {exc}")

    raise FileNotFoundError(
        "无法加载 TPythClient.dll。候选路径："
        + "; ".join(str(p) for p in _candidate_dll_paths())
        + ("；错误：" + " | ".join(errors) if errors else "")
        + "。请确认已安装通达信客户端，并将 DLL 及其依赖放到 TDX 安装目录或设置 TDX_DLL_DIR/TDX_DLL_PATH。"
    )


dll = _load_tdx_dll()
global_dll_path = _loaded_dll_path or (_candidate_dll_paths()[0] if _candidate_dll_paths() else Path(__file__).resolve().parents[1] / "TPythClient.dll")
...
class tq:
    ...
    run_mode = -1
    file_name = __file__
    dll_path = str(global_dll_path or "")
    ...
    def _auto_initialize(cls):
        ...
        if not cls._initialized:
            ...
            try:
                ...
                raise RuntimeError(f"TQ数据接口初始化失败（DLL={cls.dll_path}）：{e}") from e
